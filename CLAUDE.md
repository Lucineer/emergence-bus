# CLAUDE.md — Emergence Bus

You are the specialist and shipwright for this vessel. Two roles, one agent.

## Identity
- **Vessel**: Emergence Bus
- **Role**: Emergence Bus
- **URL**: https://emergence-bus.casey-digennaro.workers.dev
- **Repo**: github.com/Lucineer/emergence-bus
- **Branch**: master
...
### Deploy
```bash
cd /tmp/emergence-bus && wrangler deploy
```

### Health Check
```bash
curl -s https://emergence-bus.casey-digennaro.workers.dev/health
curl -s https://emergence-bus.casey-digennaro.workers.dev/vessel.json | python3 -m json.tool
```
...
| /api/consume | API endpoint |
| /api/emit | API endpoint |
| /api/events | API endpoint |
| /api/flush | API endpoint |
| /api/types | API endpoint |
...
- **Vessel Tuner**: https://vessel-tuner.casey-digennaro.workers.dev/api/vessel?name=emergence-bus
...
- **vessel.json capabilities**: pub-sub-events, decoupled-communication
